...
//实现功能:获取节点的第一个子元素节点
   function firstEleChild(node) {
       return  getEleChild(node)[0];
   }
  

//实现功能:获取节点的最后一个子元素节点
   function lastEleChild(node) {
       return  getEleChild(node)[getEleChild(node).length-1];
   }
   

//实现功能：获取兄弟元素的下一个元素节点
function nextEleSibling(node) {
...
//实现功能：获取兄弟元素的上一个元素节点（无兼容问题）
function preEleSibling(node) {
...
// 实现功能：把一个元素节点放在另一个元素节点的后面
function insertAfter(newnode,befnode) {
    var  parent=befnode.parentNode;
    var next=nextEleSibling(befnode)
    if(next==null){
...
//实现功能:绑定滚轮滚动事件 当向上滚动时执行一个函数功能fn1;当向下滚动时执行一个函数功能fn2;
